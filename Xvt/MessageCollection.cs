...
namespace Idmr.Platform.Xvt
{
	...
	public class MessageCollection : Common.ResizableCollection<Message>
	{
		...
		/// <exception cref="ArgumentOutOfRangeException"><i>quantity</i> is less than 0 or greater than <see cref="Common.ResizableCollection{T}.ItemLimit"/></exception>
		public MessageCollection(int quantity)
		{
            ...
            // kcc alt handler - warn when messages above limit, load file anyways
            if (quantity < 0) throw new ArgumentOutOfRangeException("quantity", "Invalid quantity, must be 0-" + _itemLimit);
            else if (quantity > _itemLimit)
            {
                Console.WriteLine(quantity + " is above message limit of " + _itemLimit + "; only loading first sixteen messages");
            }
            _items = new List<Message>(_itemLimit);
