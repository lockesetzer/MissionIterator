...
namespace Idmr.Platform.Xvt
{
	...
	public class FlightGroupCollection : Common.ResizableCollection<FlightGroup>
	{
		...
		public FlightGroupCollection(int quantity)
		{
            ...
            // kcc alt handler, load anyways even though FG count is above limit
            if (quantity < 1) throw new ArgumentOutOfRangeException("quantity", "Invalid quantity, must be 1-" + _itemLimit);
			else if (quantity > _itemLimit) { Console.WriteLine("Too many Flight Groups - has " + quantity + ", should be at or below " + _itemLimit); }
            _items = new List<FlightGroup>(_itemLimit);
